...
func (d *managedDatabase) Start(ctx context.Context) error {
	...
	if err := d.syncPassword(ctx); err != nil {
		return err
	}
...
func (d *managedDatabase) syncPassword(ctx context.Context) error {
	if strings.TrimSpace(d.password) == "" {
		return nil
	}
	command := exec.CommandContext(ctx, d.binary("postgres"), "--single", "-D", d.dataDir, "postgres")
	command.Stdin = strings.NewReader("ALTER ROLE " + quoteIdentifier(d.user) + " WITH PASSWORD " + quoteLiteral(d.password) + ";\n")
	if output, err := command.CombinedOutput(); err != nil {
		return fmt.Errorf("sync postgres user password: %w: %s", err, strings.TrimSpace(string(output)))
	}
	return nil
}
