...
const deviceProtocol = "athena.agent.v2"
...
func (d *deviceRuntime) execute(ctx context.Context, action deviceAction, progress func(deviceProgress)) deviceObservation {
	...
	} else if browserChallengeDetected(state) {
		base.Status, base.Error = "WAITING_USER", browserChallengeMessage(state)
...
func (d *deviceRuntime) executeCapability(ctx context.Context, action deviceAction, progress func(deviceProgress)) (map[string]any, string, error) {
	...
	case "browser.open", "browser.navigate", "browser.click", "browser.type", "browser.press", "browser.scroll", "browser.wait", "browser.download", "browser.screenshot", "browser.close", "browser.observe":
		...
		result, err := d.bridge.browser.runAction(ctx, browserExecuteRequest{
			...
			Progress: func(update browserActionProgress) {
				if progress == nil {
					return
				}
...
func browserSessionTargetKey(arguments map[string]any) string {
	...
	for _, key := range []string{"target", "url", "query", "engine"} {
