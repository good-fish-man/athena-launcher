...
func launchDesktop(opts options) error {
	...
	if stateErr != nil || selectionErr != nil {
		...
		tracker.awaitDeployment(selection)
	}
...
func stopLegacyLauncher(home string) error {
	...
	if startupCenterHealthy() {
		return fmt.Errorf("browser-based Athena did not stop; inspect %s", filepath.Join(home, "logs", "launcher.log"))
	}
	return nil
}
