...
Launcher 与服务可以使用不同版本。例如当前 Launcher 为 `v0.1.3`，服务为 `v0.1.2`：

1. 在 `agent-runtime` 发布相同 Tag。
2. 在 `agent-runtime-client` 发布相同 Tag。
3. 在 `athena-agent-ui` 发布相同 Tag。
4. 推送 Launcher 的发布 Tag。运行 **Release** 时分别填写 `tag=v0.1.3` 和 `service_tag=v0.1.2`；工作流会收集对应服务产物、打包 PostgreSQL、计算 Hash 并把 `release-manifest.json` 发布到 Launcher Release。
