...
Launcher and service releases may use different versions. For the current unified release, Launcher and services both use `v0.1.5`:
...
4. Push the Launcher release tag. Run **Release** with `tag=v0.1.5` and `service_tag=v0.1.5`; it collects those service assets, packages PostgreSQL, calculates hashes, and publishes `release-manifest.json` to the Launcher release.
